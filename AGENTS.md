# AUPostDomainExpert — Codex Session Context
...
AC Text
  ↓
1. Codex extracts testable scenarios (JSON array)
  ↓ (per scenario)
2. Domain Expert consultation — Codex queries domain RAG + code RAG,
...
3. Code RAG — automation POM + backend API context fetched
  ↓
4. Codex plans: nav_clicks[], look_for[], api_to_watch[], plan sentence
...
7. Codex decides next action OR gives verdict OR asks QA
  ↓
✅ pass / ❌ fail / ⚠️ partial / 🔶 qa_needed  per scenario
```

### Actions Available to Codex
...
- `qa_needed` — Codex is genuinely stuck, asks QA a question
...
Flow Codex should follow for field-level verification:
...
|---------|-------|-----------|
| Deep reasoning, AC verifier | Codex-sonnet-4-6 | `CLAUDE_SONNET_MODEL` |
| Fast/lightweight tasks | Codex-haiku-4-5-20251001 | `CLAUDE_HAIKU_MODEL` |
...
CLAUDE_SONNET_MODEL=Codex-sonnet-4-6
CLAUDE_HAIKU_MODEL=Codex-haiku-4-5-20251001
