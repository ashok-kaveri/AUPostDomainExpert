...
_LOCATE_PROMPT = dedent("""\
    You are a senior engineer analysing a bug report from the Australia Post Shopify App.
...
_DOMAIN_EXPERT_PROMPT = dedent("""\
    You are a domain expert for the Australia Post Shopify App (PluginHive).
...
_WEB_RESEARCH_PROMPT = dedent("""\
    You are a Australia Post Shopify App expert. A QA engineer asked:
    "{question}"
    ...
    Focus on Australia Post shipping + Shopify app behaviour.
""")
...
def ask_domain_expert(
    ...
    Answer a QA question using:
      1. QA knowledge RAG (AU Post docs)
    ...
    if verdict == "unsure":
        try:
            ...
            search_query = f"Australia Post Shopify App {question}"
