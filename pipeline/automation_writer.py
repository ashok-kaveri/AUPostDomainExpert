...
SKILL_MD  = CODEBASE / "auPostSkill.md"
...
def capture_browser_elements(
    ...
    if app_path:
        target_url = f"{app_base}/australia-post-shipping/{app_path}"
    else:
        target_url = f"{app_base}/australia-post-shipping"
...
ADD_TO_EXISTING_POM_PROMPT = dedent("""\
    You are a senior Playwright TypeScript automation engineer for the Australia Post Shopify App.
...
REVIEW_PROMPT = dedent("""\
    Review this Playwright TypeScript file for the Australia Post Shopify App.
    Check:
...
def _query_domain_expert(card_name: str, test_cases: str) -> tuple[str, list[str]]:
    ...
    Priority:
      1. Automation code RAG  — real spec files, POM classes, helper imports
         (source_type="automation" in aupost_code_knowledge collection)
      2. QA knowledge RAG     — fallback if automation not yet indexed
         (aupost_knowledge collection)
