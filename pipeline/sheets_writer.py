...
to the correct tab in the AUPost Master Sheet on Google Sheets.
...
Setup:
  ...
  2. Save as: AUPostDomainExpert/credentials.json
...
class TestCaseRow:
    ...
    transaction_id: str = ""
    pass_fail: str = ""
    release: str = ""         # e.g. "AUPostapp 2.3.115"
...
def _extract_given_when_then(tc_text: str) -> str:
    ...
    Matches the AUPost Master Sheet description column format:
...
def append_to_sheet(
    ...
    Args:
        ...
        release:              Release version string, e.g. "AUPostapp 2.3.115"
...
# Columns that match the AU Post Release sheet layout visible in the team doc
...
def _extract_ticket(desc: str, labels: list[str]) -> str:
    """
    Try to find a Jira-style ticket ID (e.g. AUPOST-123) in desc or labels.
...
def create_release_sheet(
    ...
    Args:
        release_name:  e.g. "AUPostapp 2.3.115" — used as the sheet tab name
