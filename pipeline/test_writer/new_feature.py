...
Flow:
  ...
  4. Writes files to the aupost-test-automation repo
...
Usage:
    ...
    result = generate_new_feature_tests(
        card_name="AU Post Extra Cover toggle",
        acceptance_criteria="...",
        app_url="https://your-store.myshopify.com/admin/apps/australia-post",
    )
"""
...
SPEC_GENERATOR_PROMPT = dedent("""\
    You are a senior test automation engineer for the Australia Post Shopify App.
...
UI_OBSERVER_PROMPT = dedent("""\
    You are examining a screenshot of the Australia Post Shopify App admin panel.
...
def _sanitise_path(card_name: str) -> tuple[str, str]:
    """Convert 'AU Post Extra Cover toggle' → ('holdAtLocation', 'HoldAtLocation')"""
