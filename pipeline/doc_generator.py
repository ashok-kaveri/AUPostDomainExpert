...
Usage:
    ...
    result = generate_feature_doc(
        card_name="AU Post Extra Cover",
        acceptance_criteria="...",
        test_cases="...",
        spec_file="tests/holdAtLocation/holdAtLocation.spec.ts",
        release="AUPostapp 2.3.115",
    )
    # result["doc_path"]     → "docs/features/aupost-extra-cover.md"
...
DOC_PROMPT = dedent("""\
    ...
    the Australia Post Shopify App Playwright automation suite.
    ...
    **POM:** `{pom_file}`
    ...
    ## Known Constraints
    (API limits, AU Post account requirements, Shopify restrictions — from KB context below)
...
def generate_feature_doc(
    ...
    try:
        ...
        if CHANGELOG.exists():
            ...
            CHANGELOG.write_text(
                f"# AU Post Automation — Changelog\n{changelog_entry}",
                encoding="utf-8",
            )
