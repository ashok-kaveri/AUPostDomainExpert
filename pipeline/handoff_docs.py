# Handoff document generation for the AU Post Shopify app.
# Generates Support Guides and Business Briefs from Trello card context,
# and renders them as branded PDFs.
...
# ── AU Post app navigation structure ────────────────────────────────────────
...
_AUPOST_APP_NAV = """
APP NAVIGATION STRUCTURE (Australia Post Shopify App):
───────────────────────────────────────────────────────
The AU Post app is embedded inside Shopify admin as an iframe (src matches qa-aupost.pluginhive.io).

App sidebar sections (INSIDE iframe):
  • Shipping   — All Orders grid; tabs: All | Pending | Label Generated | Manifest Completed | Returns
  ...
  • Settings   — /apps/aupost-qa/setting (SINGULAR — NOT /settings)
                 Sections: Account Settings | Packaging | Product Settings | Rates Settings |
                           Documents/Labels | Packing Slip | Additional Settings |
                           International Shipping | Pickup Settings | Shop Contact Details
  • Products   — /apps/aupost-qa/products — map products to dimensions, Signature, ATL, Insurance, DG
  • PickUp     — /apps/aupost-qa/pickup — schedule Australia Post pickup
  • Rates Log  — /apps/aupost-qa/rateslog — historical rate request / response JSON logs
  • Manifest   — /apps/aupost-qa/manifest — generate manifest
  • FAQ        — /apps/aupost-qa/faq — help articles

Shopify admin sidebar (OUTSIDE iframe):
  • Orders    — Shopify orders list (click order → More Actions to reach the AU Post app)
  • Products  — Shopify product catalog

Two account types affect which features are available:
  • eParcel       — domestic + international; Extra Cover up to $5,000; Dangerous Goods supported
  • MyPost Business — domestic only; Extra Cover up to $1,000; NO Dangerous Goods

Manual Label Generation flow:
  Shopify Orders → click order row → More Actions (button on page) → "AU Post Generate Label" (link)
  → App label page opens in iframe:
    LEFT panel  : Generate Packages → Get Shipping Rates → select rate radio button
    RIGHT panel : SideDock (always visible — configure BEFORE clicking Generate Label):
                  1. Signature: "Request Signature?" checkbox
                  2. Authority to Leave: "Authority to Leave" checkbox (cannot combine with Signature)
                  3. Insurance/Extra Cover: "Insure package" checkbox → "Insurance Details" dialog
                     → "Declared Value" spinbutton (max $5,000 eParcel / $1,000 MyPost Business)
                  4. Safe Drop: safe drop related checkbox
                  5. Dangerous Goods: dangerous goods checkbox (eParcel domestic only)
  → "Generate Label" button → redirects to Order Summary page

Order Summary page buttons:
  Print Documents | More Actions ▾ (Download Documents | Cancel Label | Return Label | How To)
  Tabs on page: Packages | Return packages
  Back button → Shipping grid (All Orders)

Return Label:
  Way A: Order Summary → "Return packages" tab → "Return Packages" button → Refresh Rates → select → Generate Return Label
  Way B: Shopify Orders → order → More Actions → "Au Post Return Label" (link)

Product configuration (for special services):
  App sidebar → Products → search product → click product row
  Fields: Length / Width / Height (cm) + Weight (kg)
  Special services: "Is Signature Needed" dropdown | "Authority to Leave" checkbox |
                    "Insure package" + "Declared Value $" | "Is Dangerous Goods" checkbox
  → Save button → toast: "Products Successfully Saved"

Rate Logs:
  During label flow: click ⋯ menu → "View Logs" → dialog shows JSON request (left) + response (right)
  After label generated: Order Summary → More Actions → Download Documents → ZIP with JSON files
"""
...
def _check_code_index() -> dict:
    ...
    new-feature navigation. Returns dict with keys:
      ...
      frontend_sync, backend_sync — {"commit": str, "timestamp": str}
...
def _fetch_code_context(card_name: str, ac_text: str = "") -> str:
    ...
    try:
        ...
        # Frontend first — has button labels, route paths, React components
...
- Add a ⚠️ QA NOTE block at the TOP of the document (before Brief Description), formatted exactly as:
...
## Where to Find It
- Australia Post app path: derive from AC / card context
...
## Value Statement
This change improves merchant workflow and support readiness for the Australia Post feature.
...
PDF_BRAND = "PluginHive Australia Post"
