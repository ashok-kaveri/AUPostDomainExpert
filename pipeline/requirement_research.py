"""
Requirement research helper for AU Post User Story / AC generation.

Combines local RAG with best-effort public web research so product requests
include official Australia Post constraints first, then PluginHive/app behaviour.
"""
...
_AUPOST_OFFICIAL_DOMAINS = (
    "auspost.com.au",
    "developers.auspost.com.au",
    "auspost.com.au/business/shipping",
)
...
def _local_research(query: str) -> str:
    """Prefer official Australia Post RAG, then PluginHive/app implementation knowledge."""
    try:
        from rag.vectorstore import search_filtered

        aupost_sections: list[str] = []
        pluginhive_sections: list[str] = []

        for source_type, bucket in (
            ("aupost_rest", aupost_sections),
        ...
        sections: list[str] = []
        if aupost_sections:
            sections.append("Official Australia Post findings from local RAG:\n" + "\n".join(aupost_sections[:3]))
...
def _web_search(
    ...
    search_query = (
        f"Australia Post {query} limits requirements API rules carrier restrictions "
        f"{site_filter}"
    )
...
def build_requirement_research_context(feature_request: str) -> str:
    ...
    Claude prompts. It includes official Australia Post evidence first, then PluginHive.
    """
    ...
    if not parts:
        return (
            "No additional official Australia Post or PluginHive research findings were available. "
    ...
    return (
        ...
        "1. Official Australia Post docs/API/carrier rules are authoritative for carrier limits.\n"
